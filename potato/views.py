...
def feedback(request):
    if request.method=="POST":
        ...
        message = 'We are happy you could take some time to visit our website. Your feedbacks are immensly valuable to us.' + '\n' + 'Thank you. Visit again' + '\n' + 'Team Leafy' + '\n' + 'Jorhat, India'
        try:
...
def chat(request):
        ...
        os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = os.path.join(settings.BASE_DIR,'chatbot-kovqhd-787be5a40d48.json')
        SESSION_ID = 'abcd12'
...
#The function name is misleading it is actually called when Detect is clicked
def index(request):
    if(request.method=='GET'):
        if request.user.is_anonymous:
            ...
            history = HistImage.objects.all().filter(user=request.user).order_by('id').reverse()
            args = {}
...
#Takes to a separate page to view the remedy from history
...
def faq(request):
    if request.method=="GET":
        ...
        questions = Question.objects.annotate(num_ans=Count('answer')).order_by('datetime').reverse()
        paginator = Paginator(questions, 10) # Show 10 contacts per page.
        ...
        context={'title' : 'QnA', 'tags': tags, 'page_obj': page_obj, 'user':request.user, 'num_qs':num_qs, 'num_pos': num_pos, 'avatar': avatar, 'profileImage':profileImage}
        ...
        if tag=='show all':
            ...
            questions = Question.objects.annotate(num_ans=Count('answer')).order_by('datetime').reverse().filter(tag_id=tag)
...
def getans(request, id):
        ...
        if questions.count() == 0:     
            raise Http404("That is an invalid question")
...
def postans(request, id):
    if request.method=="GET":
        ...
        context={'title' : 'Post answers', 'form': form, 'id':id, 'question' : question, 'avatar' : avatar}
...
def sendMail(request):
    ...
    try:
        send_mail(sub, bod, settings.EMAIL_HOST_USER, [mail], fail_silently = True)
    except:
